      ...
      <v-layout row wrap>
        <v-flex xs12 sm10 offset-sm1 md10 offest-md1 lg10 offset-lg1>
          <v-toolbar flat color="grey lighten-2">
            ...
            <th width="20%">
              <v-autocomplete clearable v-bind:items="branch" v-model="SetBranch" label="Branch:" item-text="shortCode"
                item-value="text" id="SelBranch"></v-autocomplete>
            </th>

            <v-spacer></v-spacer>
            <v-select :items="bill_status" v-model="SetStatus" label="Bill Status" @change='statuschange' id="SelEntity"
              item-text="shortCode" item-value="text"></v-select>

            <v-spacer></v-spacer>
            <v-select :items="date_type" v-model="Setdatetype" label="Date type" id="Seldate" item-text="shortCode"
            ...
            <v-menu absolute ref="menu1" :close-on-content-click="false" v-model="menu1" :nudge-right="40"
              :return-value.sync="fromdate" lazy transition="scale-transition" offset-y full-width min-width="200px">
            ...
            <v-menu absolute ref="menu2" :close-on-content-click="false" v-model="menu2" :nudge-right="40"
              :return-value.sync="todate" lazy transition="scale-transition" offset-y full-width min-width="200px">
          ...
          <template>
            <v-card-title>
              ...
              <v-spacer></v-spacer>
              <v-spacer></v-spacer>
              <v-btn color="primary" dark @click="lock_date()">
                cut-off date
              </v-btn>
              ...
              <template slot="items" slot-scope="props">
                <tr>
                  ...
                  <td class="text-xs-right" v-if="props.item.drtApproval_status==='Pending'">
                    ...
                    <v-dialog v-model="dialogcancel[props.item.Bill_no]" persistent max-width="800px" lazy absolute
                        ...
                        <v-card-text>
                          <v-container grid-list-md>
                            <v-layout wrap>
                              ...
                              <v-flex xs12 sm6>
                                <v-textarea clearable clear-icon="cancel" label="Comments" v-model='schcomments'>
                                </v-textarea>
                              </v-flex>
                            </v-layout>
                          </v-container>
                  ...
                  <td class="text-xs-right"
                    ...
                    <v-dialog v-model="dialogexpensedate[props.item.Bill_no]" persistent max-width="400px" lazy absolute
                      :key="props.item.Bill_no">
                      <v-card>
                        <v-card-title>
                        ...
                        <v-card-text>
                          <v-container grid-list-md>
                            <v-layout wrap>
                              ...
                              <v-flex xs12 sm6>
                                <v-date-picker v-model="finexpensedate" type="month" :min="minExDate" :max="maxDate"
...
export default {
  ...
  methods: {
          ...
          if (response.data.Dataschcancelled === "cancelled") {
            ...
            if (this.SetStatus == "" && this.SetBranch == "") {
              ...
              let normalusername = JSON.parse(
                sessionStorage.getItem("fin_user")
              );
              // alert(normalusername.name);
        ...
        this.$http
          ...
          .then(response => {
            ...
            if (response.data.Datainserted === true) {
              ...
              this.approval = false;
              (this.gstin = ""), (this.panno = "");
              (this.aggcommission = ""), (this.commission = "");
              this.drtdetail = "";
              this.drtid = "";
              this.drt = null;
              this.drtcomments = "";
    ...
    apiRequestfindrtbill(fromdate, todate, SetStatus, SetBranch, Setdatatype) {
      var date3 = new Date();
      var date4 =
      ...
      let datetype = "";
      console.log(
        fromdate + " " + SetStatus + " " + SetBranch + " " + Setdatatype
      );
