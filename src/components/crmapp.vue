      ...
      <v-layout row wrap>
        <v-flex xs12 sm10 offset-sm1 md10 offest-md1 lg10 offset-lg1>
          <v-toolbar flat color="grey lighten-2">
            <v-toolbar-title>CRM Appointment</v-toolbar-title>
            ...
            <v-menu absolute ref="menu1" :close-on-content-click="false" v-model="menu1" :nudge-right="40"
              ...
              <v-text-field slot="activator" v-model="fromdate" placeholder="Select From Date" prepend-inner-icon="event"
            ...
            <v-btn rounded color="primary" dark @click="apiRequestCogsdata(fromdate, todate)">Generate</v-btn>



            <download-excel :data="json_data" :fields="json_fields" type="csv" :name="fileName"
              :fetch="downloadExcelCogsdata">
            ...
            <table class="table table-hover table-bordered" v-if="show">
              ...
              <tbody>
                <tr scope="row" v-for="(item, index) in Collectionlist">
                  ...
                  <td scope="row" style="cursor:pointer" class="font-weight-black">{{ item.CARD_SERVICE_CHARGE_AMOUNT }}
                  </td>
...
export default {
  data: () => ({
    ...
    "AppointmentBookedDateTime": "AppointmentBookedDateTime",
    "Branch": "Branch",
    "Mrn": "Mrn",
  ...
  methods: {
    ...
    apiRequestCogsdata(fromdate, todate) {
      let entity = '';
      let branch = '';
      let type = '';
        ...
        this.axios
          ...
          .then(response => {
            ...
            if (response.data != '') {
              ...
              this.fileName = `CRM_APP_Report_${fromdate}_${todate}.csv`;
