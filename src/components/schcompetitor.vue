                            ...
                            <tr>
                                ...
                                <td class="text-xs-right" v-if="(props.item.approval_status==1)">
                                    <v-btn slot="activator" small fab
                                        @click.stop="$set(dialogEdit, props.item.cd_id, true)" color="error">
                                        <v-icon>fas fa-times</v-icon>
                                    </v-btn>
                                    <v-dialog v-model="dialogEdit[props.item.cd_id]" persistent max-width="400px" lazy
...
export default {
    ...
    methods: {
        ...
        loadbranch() {
            ...
            this.axios
                .get(`${process.env.API_URL}/api-schbranch/${userid.userName}`).then(response => {
        ...
        apiGetlist(date, selbranch, Selstatus) {
            ...
            this.$http.get(`${process.env.API_URL}/api-strcompetitors/${branchvalue}/${datevalue}/${statusvalue}/${userid.name}`, requestOptions)
                .then(response => {
        ...
        rowUpdate(data, comments, status) {
            ...
            this.axios.put(`${process.env.API_URL}/api-strcompetitors/${data.cd_id}`, {
                ...
                if (response.data.code === 200) {
                    this.snackbartext = 'Data Updated Submitted successfully'
                    this.snackbar = true
