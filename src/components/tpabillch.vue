        ...
        <v-flex xs12 sm10 offset-sm1 md10 offest-md1 lg10 offset-lg1>
          <v-toolbar flat color="grey lighten-2">
            ...
            <v-menu absolute ref="menu1" :close-on-content-click="false" v-model="menu1" :nudge-right="40"
              ...
              <v-date-picker color="primary" v-model="fromdate" type="month" no-title scrollable :min="minDate"
          ...
          <template>
            ...
            <v-card-title>
              ...
              <v-spacer></v-spacer>
              <v-text-field v-model="search" v-if="tpabilldata" append-icon="mdi-magnify" label="Search" single-line
		   ...
		   <span id="tpaBillPrevTemp" ref="tpaBillPrevTemp" style="width:800px"></span>
            ...
            <v-data-table :headers="headers" :items="tpabilldata" :search="search" class="elevation-4">
              <template slot="items" slot-scope="props">
                ...
                <td class="text-xs-left" style="white-space: nowrap;">{{ props.item.BILLEDDATE }}</td>
                ...
                <td class="text-xs-right" v-if="(props.item.SEND_DATE === null)">
                  <v-btn v-if="user_id != 102301" slot="activator" small fab color="success"
                    @click="rowApprove(props.item)">
                    <v-icon>check</v-icon>
                  </v-btn>
...
export default {
  ...
  methods: {
    ...
    tpaBillPrinPreview(item) {
      ...
      this.axios.get(`https://mis.dragarwal.com/api-tpabillprint/${item.BILL_ID}/${item.BILLED}/${item.AGENCY_NAME}`).then(
        response => {
          this.isLoading = true;
